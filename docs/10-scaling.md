...
Big codebases are one axis; **big screens** are another. `view(model)` returns a
fresh surface each frame and the runtime streams only the delta — the diff and
wire are already O(changed) — but a naïve `view()` still *rebuilds* the whole
tree. For large or high-frequency screens, `memo`, `list`, and `list_versioned`
make `view()` O(changed) too, so a 10 000-row screen under a ticking clock costs
the same as a 1-row screen (~11 µs/frame).

That's a topic of its own — see the dedicated **[Performance](24-performance.md)**
guide for the model, the primitives, and the measured numbers.
